...
public class CrearMenu {
	...
	public String generarMenu(int numEmpleado, List<Permiso> permiso, List<Menu> listaM, List<Menu> listaSM, List<Menu> listaSSM) {
		...
		List<Menu> listaMenu = CrearMenu.permisoMenu(permiso, listaM);
		List<Menu> listaSubMenu = CrearMenu.permisoMenu(permiso, listaSM);
		List<Menu> listaSubSubMenu = CrearMenu.permisoMenu(permiso, listaSSM);
		
		String menuCompleto = CrearMenu.concatenarMenu(listaMenu, listaSubMenu, listaSubSubMenu);
		
		return menuCompleto;
	}
	
	public static List<Menu> permisoMenu(List<Permiso> permiso, List<Menu> lista){
		// Se identifica que el usuario tenga permisos para ver las opciones del menu
		List<Menu> listaMenu = new ArrayList<Menu>();
		int conta = 0;
		for(int contador=0;contador<lista.size();contador++) {
			for(int cont=0;cont<permiso.size();cont++) {
				if((permiso.get(cont).getMenu().getIdMenu()==lista.get(contador).getIdMenu()) && permiso.get(cont).getEstatusPermiso().getIdEstatus()==1){
					listaMenu.add(conta, lista.get(contador));
					conta++;
				}
			}
		}
	...
	public static String concatenarMenu(List<Menu> listaMenu, List<Menu> listaSubMenu, List<Menu> listaSubSubMenu/*, falta agregar la tabla de los permisos*/) {
	...
	// Se deja metodo para su posterior eliminacion
